...
class LocationPickView extends ConsumerStatefulWidget {
  final bool isAddingAddress;
  final VoidCallback? onBack;
  final bool isEmbedded;
  final Function(LatLng, DetailedAddress)? onConfirm;
  ...
  @override
  ConsumerState<LocationPickView> createState() => _LocationPickViewState();
}

class _LocationPickViewState extends ConsumerState<LocationPickView> {
...
[
  ...
  { "featureType": "poi.park", "elementType": "geometry", "stylers": [ { "color": "#e8f6f1" } ] },
  { "featureType": "poi.park", "elementType": "labels.text.fill", "stylers": [ { "color": "#1ba672" } ] },
    ...
    return Stack(
      children: [
        ...
        IgnorePointer(
          child: Center(
            child: Column(
              ...
              children: [
                const Icon(Icons.location_on, color: AppColors.brandGreen, size: 44),
                Container(
                  width: 8, height: 8,
        ...
        Positioned(
          ...
          child: widget.isEmbedded ? const SizedBox.shrink() : SafeArea(
            child: Padding(
                      ...
                      child: Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 12),
                            Text('Search an area or address', style: GoogleFonts.poppins(color: AppColors.textSub, fontSize: 13, fontWeight: FontWeight.w500)),
                          ],
                        ),
                      ),
                    ),
        ...
        Positioned(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                if (!isAtCurrent) ...[
                  Text('Order will be delivered here', style: GoogleFonts.poppins(color: AppColors.textSub, fontWeight: FontWeight.w500, fontSize: 13)),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            _currentAddress?.area ?? 'Detecting...',
                            style: GoogleFonts.poppins(fontWeight: FontWeight.w700, fontSize: 17, color: AppColors.textMain),
                            maxLines: 1, overflow: TextOverflow.ellipsis,
                          ),
                          const SizedBox(height: 2),
                          Text(
                            _currentAddress?.formattedAddress ?? 'Locating...',
                            style: GoogleFonts.poppins(color: AppColors.textSub, fontSize: 13, fontWeight: FontWeight.w400, height: 1.3),
                ...
                if (!isAtCurrent) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(color: const Color(0xFFFFF7E6), borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0xFFFFE7BA))),
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        const Icon(Icons.info_outline, color: AppColors.ctaOrange, size: 14),
                        const SizedBox(width: 8),
                        Text('This is ${_getDistanceString()} away from you', style: GoogleFonts.poppins(color: AppColors.ctaOrangePressed, fontWeight: FontWeight.w500, fontSize: 11)),
                      ],
                    ),
                  ),
                ],
                ...
                ElevatedButton(
                  onPressed: _isLoadingAddress || _currentAddress == null ? null : () {
                    if (widget.isEmbedded && widget.onConfirm != null) {
                      ...
                      widget.onConfirm!(_currentCenter!, _currentAddress!);
                    } else {
                      if (widget.isAddingAddress) {
                        Navigator.push(context, MaterialPageRoute(builder: (_) => AddAddressDetailScreen(position: _currentCenter!, address: _currentAddress!)));
                      } else {
                        final newLoc = LocationData(
                          city: _currentAddress!.city ?? '',
                          state: _currentAddress!.state ?? '',
                  ...
                  child: _isLoadingAddress 
                    ...
                    : Text('Confirm & proceed', style: GoogleFonts.poppins(fontWeight: FontWeight.w700, fontSize: 15)),
                ),
              ],
            ),
          ),
